...
export function pitchZonePct(pitch: PitchEvent): { left: number;
	bottom: number } | null
{
	...
	return plateToStrikeZonePct(
		{
			pX: data.coordinates.pX,
			pZ: data.coordinates.pZ,
		},
		{
			top: data.strikeZoneTop,
			bottom: data.strikeZoneBottom,
		},
		{
			padHorizontalFt: 0.6,
			padVerticalFt: 0.4,
