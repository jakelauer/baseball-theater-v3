...
export function StrikeZoneChart({ pitches, selectedPitchNumber, onSelectPitch }: Props) {
  const withLocation = pitches
    .map((pitch) => ({ pitch, pct: pitchZonePct(pitch) }))
    .filter(
      (p): p is { pitch: PitchEvent; pct: { left: number; bottom: number } } => !!p.pct,
    );
