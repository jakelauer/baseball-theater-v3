...
import { kinematicsFromPitch, pitchDotColor } from "./utils";
...
type ViewBox = { minX: number; maxX: number; minY: number; maxY: number };

function mapPath(
  points: TrajectoryPoint[],
  map: (p: TrajectoryPoint) => { x: number; y: number },
  vb: ViewBox,
  width: number,
  height: number,
): string {
...
export function PitchTrajectoryView({ pitch, playing, onPlaybackEnd }: Props) {
  const [progress, setProgress] = useState(0);

  const { pitchPath, hitPath } = useMemo(() => {
    if (!pitch?.pitchData)
      return { pitchPath: [] as TrajectoryPoint[], hitPath: [] as TrajectoryPoint[] };
  ...
  const topPath = mapPath(
    pitchPath,
    (p) => ({ x: p.x, y: p.z }),
    { minX: -2.5, maxX: 2.5, minY: 0, maxY: 8 },
    280,
    160,
  );
    ...
    <Box>
      ...
      <Group align="flex-start" gap="md" wrap="wrap">
        <TrajectoryPanel
          ...
          height={160}
          pathD={sidePath}
          ball={pitchPoint ? { x: pitchPoint.y, y: pitchPoint.z } : null}
          vb={{ minX: 0, maxX: 55, minY: 0, maxY: 8 }}
        />
        <TrajectoryPanel
          ...
          height={160}
          pathD={topPath}
          ball={pitchPoint ? { x: pitchPoint.x, y: pitchPoint.z } : null}
          vb={{ minX: -2.5, maxX: 2.5, minY: 0, maxY: 8 }}
        />
      </Group>
      {pitch.hitData && pitch.details.isInPlay ? (
        <Box mt="md">
          ...
          <svg width={280} height={160} style={{ display: "block", borderRadius: 8 }}>
