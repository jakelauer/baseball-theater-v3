...
export async function handleApiRequest(
  ...
  if (req.method === "GET" && pathname === "/health") {
    sendJson(res, 200, { ok: true, service: "bt-functions-local" });
    return;
  }
  ...
  if (req.method === "GET" && pathname === "/api/schedule") {
    ...
    if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      sendJson(res, 400, { error: "date_required", hint: "YYYY-MM-DD" });
      return;
    }
    const force = url.searchParams.get("refresh") === "1";
    const day = await getOrRefreshSchedule(ctx.ingest, date, { force });
  ...
  if (req.method === "GET" && gameMatch) {
    ...
    const force = url.searchParams.get("refresh") === "1";
    const game = await getOrRefreshGame(ctx.ingest, gamePk, { force });
    if (!game) {
      notFound(res);
      return;
    }
