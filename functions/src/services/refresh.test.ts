...
import { CACHE_TTL_MS, isStale, RefreshCoordinator } from "./refresh.js";
...
function cached(fetchedAt: string): GameSnapshot {
  return {
    ...
    teams: {
      home: { id: 120, name: "Washington Nationals", abbreviation: "WSH", runs: 0 },
      away: { id: 121, name: "New York Mets", abbreviation: "NYM", runs: 0 },
    },
...
describe("getOrRefreshGame staleness trigger", () => {
  it("does not refresh a fresh cache entry", async () => {
    ...
    const out = await getOrRefreshGame(
      deps({ mlb: spy.client, games, refresh: new RefreshCoordinator() }),
      744834,
    );
...
describe("single-flight", () => {
  it("collapses N concurrent stale reads for one key into 1 upstream call", async () => {
    ...
    const games = new InMemoryGameRepository();
    await games.upsert(cached(STALE));
    const d = deps({ mlb: spy.client, games, refresh: new RefreshCoordinator() });

    const reads = Promise.all(
      Array.from({ length: 8 }, () => getOrRefreshGame(d, 744834)),
    );
...
describe("refresh cooldown", () => {
  it("suppresses a second upstream refresh inside REFRESH_COOLDOWN_MS", async () => {
    ...
    const d = deps({ mlb: spy.client, games, refresh: cooldown, now: () => clock });
