...
13. **`docs/v3/AUDIT.md` and the backlog notes are always inside every story's fence.** The ledger entry
    ...
    in every fence — the whole directory, because rule 17's baseline and the **Next:** marker live in
...
**Next:** lowest **Priority** with Status `todo` — **16 / S27** — **after** confirming no [backlog review](#backlog-reviews) is due (the 2026-09-13 post-V3 review just ran: drift from vault-migration V3 changing `pnpm verify` and where the backlog is edited; verdict `amended`, added `docs/v3/backlog/` to every `todo` fence) **and** after vault-migration V3's final commit has landed (see **Next review due**). This line is hand-maintained in `docs/v3/backlog/frame/00 …` — unlike the table above, the build does not generate it.
...
- **New (2026-09-13 post-V3): the backlog's hand-maintained parts.** V3 generates the status table, headings, and `**Status:**` lines, but the **Next:** marker, this scheduling section, and the Current baseline are still prose in `frame/` notes that a finishing story must edit by hand. They are now inside every `todo` fence; the next review should check the **Next:** marker again, since it has gone stale before.
