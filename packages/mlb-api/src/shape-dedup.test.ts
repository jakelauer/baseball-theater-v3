...
function readInterfaces(): ExportedInterface[]
{
	...
	))
	{
		...
		for (const line of lines)
		{
			...
			// One tab = one indent level (see .prettierignore / eslint.config.js):
			// this must stay a single level so a nested inline type's fields don't
			// get counted as the enclosing interface's own fields.
			const field = /^\t(?:readonly )?([A-Za-z0-9_]+)\??:/.exec(line);
