...
const gameDiffPatchResponseSchema = z.array(
  z.object({ diff: z.array(jsonPatchOpSchema) }),
);
...
export function reconstructReplay(
  ...
  for (const entry of walk) {
    ...
    states.push({ timecode: entry.endTimecode, feed: clone(current) });
  }

  return { timecodes: states.map((state) => state.timecode), states };
}
