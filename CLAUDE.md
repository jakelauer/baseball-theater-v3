...
- Update backlog **Status** in the same change set: `doing` when you start, `done` when every AC passes (never leave it stale after finishing). Keep the top **Story status** table in sync and sorted by **Priority** (IDs are labels, not order). Keep the **Current baseline** table and its **As of** line in sync too (`docs/v3/BACKLOG.md` rule 17) — whenever a story flips to `done` or is inserted/removed/re-scoped.
